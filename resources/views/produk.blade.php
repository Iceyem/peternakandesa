...
<script>
  document.addEventListener('DOMContentLoaded', async function() {
    try {
      const response = await fetch('http://localhost/apipeternakandesa/readsapibali.php');
      if (!response.ok) {
      ...
      const productList = document.getElementById('sapibali');
      ...
      data.forEach(product => {
        ...
        productItem.innerHTML = `
          ...
          <div class="card">
            <img src="assets/sapi1.jpg" class="card-img-top" alt="...">
            <div class="card-body">
...
<script>
  document.addEventListener('DOMContentLoaded', async function() {
    try {
      const response = await fetch('http://localhost/apipeternakandesa/readkambingdomba.php');
      if (!response.ok) {
        throw new Error('Failed to fetch data');
      }
      const text = await response.text();
      ...
      const productList = document.getElementById('kambingdomba');
      productList.innerHTML = '';
      data.forEach(product => {
        const productItem = document.createElement('div');
        productItem.classList.add('col-md-4', 'mb-4');
        productItem.innerHTML = `
          <div class="card-container">
          <div class="card">
            <img src="assets/kambing3.jpeg" class="card-img-top" alt="...">
            <div class="card-body">
              <h5 class="card-title">${product.namaproduk}</h5>
              <p class="card-text">Berat: ${product.berat} kg</p>
              <p class="card-text">Harga: Rp ${product.harga}</p>
...
<script>
  document.addEventListener('DOMContentLoaded', async function() {
    try {
      const response = await fetch('http://localhost/apipeternakandesa/readtelurpuyuh.php');
      if (!response.ok) {
        throw new Error('Failed to fetch data');
      }
      const text = await response.text();
      ...
      const cleanJson = text.substring(startIndex);
      const data = JSON.parse(cleanJson);
      const productList = document.getElementById('telurpuyuh');
      productList.innerHTML = '';
      data.forEach(product => {
        const productItem = document.createElement('div');
        productItem.classList.add('col-md-4', 'mb-4');
        productItem.innerHTML = `
          <div class="card-container">
          <div class="card">
            <img src="assets/puyuh4.jpeg" class="card-img-top" alt="...">
            <div class="card-body">
              <h5 class="card-title">${product.namaproduk}</h5>
              <p class="card-text" style="margin: 0;">Berat: ${product.berat} kg</p>
              <p class="card-text">Isi: ${product.isi} Butir</p>
              <p class="card-text">Harga: Rp ${product.harga}</p>
            </div>
          </div>
        `;
